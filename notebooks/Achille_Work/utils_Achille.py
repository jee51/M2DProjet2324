...
'''
    ...
    Changelog 25/01:
        1. Modified the get_consumption() function to:
    ...
    for i, df in enumerate(ac):
        ...
        if len(df.columns)>0 and "ALT [ft]" in df.columns:
            ...
            if Alt_max > 20000: # Maxmimum altitude can indicates if a recording is meaningful or not
                ...
                dat+= [['AC'+ac.storename[-5:-3], 'Left', i, phase, Leg, phase_duration, Alt_max, Mach_max, slope, average_egt, TAT_max, TAT_min, T_oil_range_1, volume1, total_weight]]
                dat+= [['AC'+ac.storename[-5:-3], 'Right', i, phase, Leg, phase_duration, Alt_max, Mach_max, slope, average_egt, TAT_max, TAT_min, T_oil_range_2, volume2, total_weight]]

    dataframe = pd.DataFrame(dat,columns = ['AC', 'ENG', 'Flight', 'Phase', 'Leg_flight_H', 'Leg_phase_H', 'Alt_max_Ft', 'Alt_slope', 'Avg_egt',
                                            'TAT_max', 'TAT_min', 'T_oil_range', 'M_max', 'Volume_L', 'Weight_Kg'])
...
def plot_residus(res, title):
    ...
    ax2.set_title('Diagramme de dispersion des résidus')

    ax3 = fig.add_subplot(1, 3, 3)
    stats.probplot(residus, dist="norm", plot=plt)
    ax3.set_xlabel('Quantiles théoriques')
    ax3.set_ylabel('Quantiles des résidus')
